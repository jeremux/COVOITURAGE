/*
 * Classe pour l'interaction avec la table profil
 *
...
public class ProfilGestionImpl extends DAO<Profil> {
	...
	public Profil findByPseudo(String pseudo) {
		...
		try {
			...
			System.err.println("Erreur findByPseudo table Profil");
			e.printStackTrace();
		}
		return p;
	}
	...
	public Profil findByEmail(String email) {
		...
		try {
			...
			System.err.println("Erreur findByEmail table Profil");
			e.printStackTrace();
		}
		return p;
	...
	public Profil find(int id) {
		...
		try {
			...
			System.err.println("Erreur find table Profil");
			e.printStackTrace();
		}
		return p;
	}
		...
		if (tmp.getId() == -1) {
			try {
				PreparedStatement ps = connection
						.prepareStatement("INSERT INTO Profil (pseudo,pass,email,dateInscription,dateNaissance,"
	...
	public Profil update(Profil p) {
		...
		try {
			PreparedStatement ps = this.connection.prepareStatement("update Profil set " + "pseudo=?," + "pass= ?,"
					+ "email= ?," + "dateInscription= ?," + "dateNaissance=?," + "nom=?," + "prenom=?," + "ville=?,"
					+ "preference=?" + "where idProfil=?");
			...
			System.err.println("Erreur update table Profil");
			e.printStackTrace();
		}
	...
	public Profil update(Profil old, Profil newProfil) {
		...
		try {
			...
			System.err.println("Erreur update Profil");
			e.printStackTrace();
		}
		return res;
	}
