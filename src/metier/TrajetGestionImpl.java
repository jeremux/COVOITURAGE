/*
 * Classe pour l'interaction avec la table Trajet
 *
...
public class TrajetGestionImpl extends DAO<Trajet> {
	...
	public Trajet find(int id) {
		...
		try {
			...
			System.err.println("Erreur find table Trajet");
			e.printStackTrace();
		}
		return t;
	}
	...
	public Trajet create(Trajet t) {
		...
		try {
			PreparedStatement ps = connection
					.prepareStatement("INSERT INTO Trajet (villeDepart,villeArrivee,date,heure,places,"
	...
	public Trajet update(Trajet t) {
		...
		try {
			PreparedStatement ps = this.connection
					.prepareStatement("update Trajet set " + "villeDepart=?," + "villeArrivee=?," + "date=?,"
							+ "heure=?," + "places=?," + "prix=?," + "idConducteur=?" + "where idTrajet=?;");
			...
			System.err.println("Erreur from table Trajet");
			e.printStackTrace();
		}
	...
	public List<Trajet> find(Ville depart, Ville arrivee) {
		...
		try {
			PreparedStatement ps = connection
					.prepareStatement("select * from Trajet where villeDepart=? and villeArrivee=?;");
	...
	public List<Trajet> find(Ville depart, Ville arrivee, String date) {
		...
		try {
			PreparedStatement ps = connection
					.prepareStatement("select * from Trajet where villeDepart=? and villeArrivee=? and date=?;");
			ps.setInt(1, depart.getId());
			ps.setInt(2, arrivee.getId());
			ps.setString(3, date);
			ResultSet rs = ps.executeQuery();
	...
	public List<Trajet> findAll() {
		...
		try {
			...
			System.err.println("Erreur findAll table Trajet");
			e.printStackTrace();
		}
