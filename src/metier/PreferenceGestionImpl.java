/*
 * Classe pour l'interaction avec la table Preference
 *
...
public class PreferenceGestionImpl extends DAO<Preference> {
	...
	public Preference find(int idPref) {
		...
		try {
			...
			System.err.println("Erreur find from table Preference");
			e.printStackTrace();
		}
		return p;
	}
	...
	public Preference getPreference(Preference p) {
		...
		try {
			PreparedStatement ps = connection
					.prepareStatement("select * from Preference where aimeMusique=? and aimeAnimaux=? and fumeur=? and "
							+ "aimeDiscution=?;");
	...
	public Preference create(Preference p) {
		try {
			PreparedStatement ps = connection
					.prepareStatement("select count(*) from Preference where aimeAnimaux=? and aimeMusique=? "
			...
			if (count == 0) {
				ps = connection.prepareStatement(
						"INSERT INTO Preference (aimeMusique,aimeAnimaux,fumeur,aimeDiscution) values (?,?,?,?);");
				...
				if (nbModif != 1) {
					System.err.println(
							"Problème: modification de plus d'une ligne lors de l'ajout dans la table Preference");
				}
			...
			System.err.println("Erreur insert into table Preference");
			e.printStackTrace();
		}
	...
	public Preference update(Preference p) {
		Preference res = new Preference();
		try {
			PreparedStatement ps = this.connection.prepareStatement("update Preference set " + "aimeMusique=?,"
					+ "aimeAnimaux= ?," + "fumeur= ?," + "aimeDiscution= ?," + "where idPreference=?;");
			...
			System.err.println("Erreur update table Preference");
			e.printStackTrace();
		}
