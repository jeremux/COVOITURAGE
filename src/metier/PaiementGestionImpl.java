...
public class PaiementGestionImpl extends DAO<Paiement> {
	...
	public Paiement find(int idPassager, int idTrajet) {
		...
		try {
			PreparedStatement ps = connection
					.prepareStatement("select * from Paiement where " + "idPassager=? and" + "idTrajet=?;");
	...
	public Paiement create(Paiement p) {
		try {
			PreparedStatement ps = connection
					.prepareStatement("INSERT INTO Paiement (idPassager,idTrajet,paiementValide) values (?,?,?);");
	...
	public Paiement update(Paiement p) {
		try {
			PreparedStatement ps = this.connection
					.prepareStatement("update Paiement set" + "idPassager=?," + "idTrajet=?," + "paiementValide=?");
	...
	public void delete(Paiement p) {
		try {
			PreparedStatement ps = connection
					.prepareStatement("delete from Paiement where idPassager=? and idTrajet=?");
			ps.setInt(1, p.getPassager().getId());
			ps.setInt(2, p.getTrajet().getId());
